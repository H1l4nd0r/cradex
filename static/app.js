class FuturesArbitrageScanner {
    ...
    setupEventListeners() {
        ...
        // Event delegation for Go buttons
        ...
        try {
            ...
            sourceNames.forEach(source => {
                ...
                if (this.isSourceEnabled(source)) {
                    ...
                    if (history.length > 0) {
                        try {
                            // Filter out any invalid data points and convert to TradingView format
                            const seriesData = history
                                .filter(([timestamp, price]) => timestamp != null && price != null && !isNaN(timestamp) && !isNaN(price))
                                .map(([timestamp, price]) => ({
                                    time: timestamp,
                                    value: parseFloat(price)
                                }))
                                .filter(point => point.value > 0); // Ensure positive values
                            ...
                            console.error(`Error setting chart data for ${source}:`, error);
                            series.setData([]);
                        }
                    } else {
    ...
    performActiveOrdersTableUpdate() {
        ...
        if (!this.activeOrders || this.activeOrders.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" class="orders-empty">No active orders...</td></tr>';
            return;
        }
        ...
        const closeButtons = tbody.querySelectorAll('.close-order-button');
